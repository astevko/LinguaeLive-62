...
public class CourseLinkDao extends ObjectifyGenericDao<CourseLink> {
    ...
    {
            Query<CourseLink> q = ofy().query(clazz);
            return q.list();
    }
...
//    	
    ...
    public List<Course> getAllLinkedCourses( Long courseId ) {
    	...
    	List<CourseLink> linkedA = ofy().query( CourseLink.class ).filter( "courseAKey =", courseKey ).list();
    	...
    	List<CourseLink> linkedB = ofy().query( CourseLink.class ).filter( "courseBKey =", courseKey ).list();
    ...
    public List<Course> getAcceptedLinkedCourses( Long courseId ) {
    	...
    	Key<Course> courseKey = new Key<Course>( Course.class, courseId );
    	List<Course> linkedCourses = new ArrayList<Course>();
    	List<CourseLink> linkedA = ofy().query( CourseLink.class ).filter("accepted =", true).filter( "courseAKey =", courseKey ).list();
    	for( CourseLink link : linkedA ) {
    		try {
				linkedCourses.add( link.getCourseB() );
			} catch (EntityNotFoundException e) {
    	...
    	List<CourseLink> linkedB = ofy().query( CourseLink.class ).filter("accepted =", true).filter( "courseBKey =", courseKey ).list();
    	for( CourseLink link : linkedB ) {
    		try {
				linkedCourses.add( link.getCourseA() );
			} catch (EntityNotFoundException e) {
	...
	public List<CourseLink> listPendingLinkedCourses( User user ){
		...
		for (Course c : courses ) {
			pending.addAll( ofy().query( CourseLink.class )
					.filter("pending =", true)
	...
	public List<CourseLink> listOpenLinkedCourses(  User user ){
		...
		// find all pending courseLinks for courses
		for (Course c : courses ) {
			open.addAll( ofy().query( CourseLink.class )
					.filter("pending =", true)
   ...
   /**
    ...
    public void deleteLinkedCourses( Long courseId ) {
    	...
    	Key<Course> courseKey = new Key<Course>( Course.class, courseId );
    	ofy().delete( ofy().query( CourseLink.class ).filter( "courseAKey =", courseKey ).list());
    	ofy().delete( ofy().query( CourseLink.class ).filter( "courseBKey =", courseKey ).list());
