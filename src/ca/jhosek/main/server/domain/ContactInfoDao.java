...
public class ContactInfoDao extends ObjectifyGenericDao<ContactInfo> {
	...
	public List<ContactInfo> findContactInfos( User user ){
		...
		List<ContactInfo> contactInfos = ofy().query(ContactInfo.class).filter("ownerKey =", user).list();
		return contactInfos;
	}
	...
	public ContactInfo findPreferredContactInfo( User user ){
		logger.info("findContactInfos( " + user.getFirstName() + " " + user.getLastName() + " ) " );
		ContactInfo contactInfo = ofy().query(ContactInfo.class).filter("ownerKey =", user).filter( "preferred =", true).get();
		return contactInfo;
	}
...
//    	}
    ...
    public ContactInfo update( ContactInfo newEntity ) throws EntityNotFoundException {
