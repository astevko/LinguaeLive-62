...
public class UserDao extends ObjectifyGenericDao<User> {
	...
	public Long countUsers() {
		final Long userCount = new Long(ofy().query(User.class).listKeys()
				.size());
	...
	private List<User> fetchUserKeys(final List<Key<User>> userKeys) {
		final Map<Key<User>, User> fetched = ofy().get(userKeys);
	...
	public List<User> findAllUsers() {
		final List<User> users = ofy().query(User.class).list();
	...
	public User findEmailAddress(String emailAddress) {
		...
		final User user = ofy().query(User.class)
				.filter("emailAddress", emailAddress).get();
		return user;
	}
	...
	public List<ContactInfo> getContactInfoForUser(final Long userId) {
		if (userId != null)
			return ofy().query(ContactInfo.class).filter("userId", userId)
					.list();
		return null;
	}
	...
	public List<User> getCourseMembers(final Course course,
		...
		Query<Member> q;
		if (availableOnly) {
			q = ofy().query(Member.class).filter("courseKey", course)
	...
	public User persist(final User user) {
		...
		final List<Key<User>> emailKeys = ofy().query(User.class)
				.filter("emailAddress", user.getEmailAddress()).listKeys();
		if (user.getId() == null && emailKeys.size() > 0)
			...
			final Key<User> userKey = new Key<User>(User.class, user.getId());
	...
	public List<String> gatherAffiliateEmailAddresses( final User user) {
		...
		for (Member myMember : myMembers) {
			final Key<Member> myMemberKey = new Key<Member>( Member.class, myMember.getId() );
