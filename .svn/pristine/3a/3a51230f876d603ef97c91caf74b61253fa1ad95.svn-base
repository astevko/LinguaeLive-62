...
public class MemberDao extends ObjectifyGenericDao<Member> {
	...
	{
		logger.info( " listAll()");
		Query<Member> q = ofy().query(clazz);
		return q.list();
	}
	...
	public Member findCourseMember( Long userId, Long courseId ){
		...
		return ofy().query( Member.class )
		.filter("courseKey", new Key<Course>( Course.class, courseId) )
		.filter("userKey",   new Key<User>(   User.class,   userId) )
		.get();
	}
	...
	public List<Member> fetchMemberKeys(final List<Key<Member>> memberKeys) {
		final Map<Key<Member>, Member> fetched = ofy().get(memberKeys);
	...
	public List<Member> getMembersOfCourse( Course course ){			
		...
		final List<Member> list =  ofy().query(Member.class).filter("courseKey", course).list();
	...
	public List<Member> getMembersOfUser( User user ){			
		logger.info( " getMembersOfCourse()");
		return ofy().query(Member.class).filter("userKey", user).list();
	...
	public List<Member> getAvailableMembersOfCourse( Course course ){			
		...
		return ofy().query(Member.class)
		.filter("courseKey =", new Key<Course>(Course.class,course.getId()) )
